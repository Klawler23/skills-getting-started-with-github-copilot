document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderParticipants(activityName, participants) {
    if (!participants || participants.length === 0) {
      return `
        <p><strong>Participants:</strong> <span class="no-participants">No participants yet.</span></p>
      `;
    }
        ...
        <ul class="participants-list">
          ${participants.map((participant) => `
            <li class="participant-item">
              ...
              <button class="participant-delete" data-activity="${escapeHtml(activityName)}" data-email="${escapeHtml(participant)}" aria-label="Remove ${escapeHtml(participant)}">
                &times;
              </button>
            </li>
          `).join("")}
  ...
  async function fetchActivities() {
    try {
      ...
      // Clear activity list and reset dropdown
